...
def entropy(t):
    return -(t * torch.log(t.clamp_min(0.0000001))).sum(dim=[-1, -2, -3]).mean()
...
class SampledCRFLoss(torch.nn.Module):
    ...
    def forward(self, guidance, features):
        ...
        norm_coords = coords / torch.tensor([h, w], device=guidance.device).unsqueeze(
            -1
        )
        ...
        coord_diff = (
            (norm_coords.unsqueeze(-1) - norm_coords.unsqueeze(-2))
            .square()
            .sum(0)
            .unsqueeze(0)
        )
        ...
        feat_diff = (
            (selected_feats.unsqueeze(-1) - selected_feats.unsqueeze(-2))
            .square()
            .sum(1)
        )
